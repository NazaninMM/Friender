/*
  # Fix join_requests updated_at trigger error

  This migration fixes the "record 'new' has no field 'updated_at'" error by:
  1. Ensuring the updated_at column exists on join_requests table
  2. Recreating the trigger function to handle the updated_at field properly
  3. Recreating the trigger on the join_requests table

  The error occurs because the trigger function is trying to access NEW.updated_at
  but the field might not be properly accessible in the trigger context.
*/
