...
import { ArrowLeft, Send, Check, X, Calendar, User, Clock, Smile } from 'lucide-react';
import { Button } from '../ui/Button';
import { Input } from '../ui/Input';
import { useAuth } from '../../hooks/useAuth';
import { joinRequestService, JoinRequestChat, JoinRequestChatMessage } from '../../lib/joinRequestService';
...
export const JoinRequestChatScreen: React.FC<JoinRequestChatScreenProps> = ({
  ...
  const loadJoinRequestStatus = async () => {
    try {
      const joinRequest = await joinRequestService.getJoinRequest(chat.joinRequestId);
      if (joinRequest) {
        setJoinRequestStatus(joinRequest.status as 'pending' | 'approved' | 'denied');
      }
    } catch (error) {
  ...
  const handleApprove = async () => {
    ...
    try {
      const success = await joinRequestService.approveJoinRequest(chat.joinRequestId, user.id);
      if (success) {
  ...
  const handleDeny = async () => {
    ...
    setProcessingAction(true);
    try {
      const success = await joinRequestService.denyJoinRequest(chat.joinRequestId, user.id);
      if (success) {
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      >
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-600">{isHost ? 'Host' : 'Requester'}</span>
        </div>
      </motion.div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {messages.map((msg, index) => {
          ...
          const isSystemMessage = ['system', 'approval', 'rejection', 'join_request'].includes(msg.messageType);
          const showAvatar = !isCurrentUser && !isSystemMessage && (index === 0 || messages[index - 1].senderId !== msg.senderId);
          ...
          return (
            <div key={msg.id}>
              {isSystemMessage ? (
                ...
                >
                  <div className={`inline-block px-4 py-2 rounded-full text-sm border ${getMessageTypeColor(msg.messageType)}`}>
                    {msg.messageText}
                  </div>
                </motion.div>
              ) : (
                ...
                >
                  ...
                  <div className={`max-w-xs lg:max-w-md ${isCurrentUser ? 'order-1' : ''}`}>
                    {!isCurrentUser && showAvatar && (
                      <p className="text-xs text-gray-600 mb-1 ml-1">{msg.sender.name}</p>
                    )}
